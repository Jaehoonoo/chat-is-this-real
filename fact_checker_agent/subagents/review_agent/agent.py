...
class AdjudicatorOutput(BaseModel): # MAYBE FIX THIS OUTPUT_SCHEMA
    ...
    """
    status: Literal["Approved", "Revision Needed"] = Field(..., description="The final decision on the report's quality.")
    feedback: Optional[str] = Field(None, description="Specific, actionable feedback for revision if the status is 'Revision Needed'.")
...
ADJUDICATOR_PROMPT = """
You are the Final Adjudicator, the last line of defense for quality and accuracy. Your mission is to review the draft final_report from the Analyst agent and decide if it meets publication standards. You are ruthlessly logical and impartial.
...
4.  **Clarity and Neutrality**: Is the `justification` written clearly, concisely, and in a neutral, impartial tone?
...
#### Final Output:
Ensure your final output is a single, valid JSON object that perfectly matches the `AdjudicatorOutput` schema.
"""
