...
function generateUUID() {
    return 'ext-' + Date.now() + '-' + Math.random().toString(36).substring(2, 15);
}
...
async function analyzePage(url) {
    ...
    const runData = {
        ...
        new_message: {
            parts: [
                {
                    text: `Use your tools to fetch the content from this URL: ${url}`
                }
            ]
        }
    };
    ...
    if (!runResponse.ok) {
        ...
        throw new Error(`API request failed with status ${runResponse.status}: ${errorText}`);
    }
